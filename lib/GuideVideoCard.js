...
// (`en/ch0/co.mp4`) is live today, so we serve English as the fallback until
// other locales are localized — matching the tutorial plan's English-first rule.
//
// The narrator-mode toggle (Puck / Aoede solo) is intentionally omitted until
// those tracks lock; when they do, add a mode switch here and swap `co` in the
// path for the chosen mode.

const SUPABASE_URL = (process.env.NEXT_PUBLIC_SUPABASE_URL || "").replace(/\/$/, "");

// `lang` is wired for later (viewer's native_lang); defaults to English, which
// is also the fallback for any locale whose tour isn't localized yet.
function tourUrl(lang = "en") {
  if (!SUPABASE_URL) return "";
  return `${SUPABASE_URL}/storage/v1/object/public/tutorial-video/${lang}/ch0/co.mp4`;
}

export default function GuideVideoCard({ lang = "en" }) {
  const [open, setOpen] = useState(false);
  const url = tourUrl(lang);
  if (!url) return null;
    ...
    <div style={styles.card}>
      {!open && (
        <button
          ...
          aria-label="Watch the video tour"
        >
          <span style={styles.thumb} aria-hidden="true">
            <span style={styles.play}>▶</span>
          </span>
          <span style={styles.headText}>
            <span style={styles.kicker}>▶ 30-second tour</span>
            <span style={styles.title}>Watch the video tour</span>
            <span style={styles.sub}>Meet the app in half a minute — the fastest way in.</span>
            <span style={styles.badge}>🔊 In English · Español next</span>
          </span>
        </button>
      )}
