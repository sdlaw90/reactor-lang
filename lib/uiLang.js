...
export function useUiLang() {
  ...
  useEffect(() => {
    // Keep <html lang> honest. app/layout.js renders lang="en" on the server (the
    // real language isn't known until localStorage/navigator are readable), so a
    // screen reader and Chrome's translate prompt would be told English while the
    // page shows French. Client-side sync is enough.
    const syncDocLang = (l) => {
      if (typeof document !== "undefined" && l) document.documentElement.lang = l;
    };
    const resolved = resolveUiLang();
    syncDocLang(resolved);
    setLang(resolved);
