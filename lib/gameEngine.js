...
export function flattenBank(bank, tagFor, sourceLang, l10nMap) {
  ...
  Object.keys(bank).forEach((cat) => {
    bank[cat].forEach((q, i) => {
      ...
      // maps item id ("cat-i") → { prompt?, promptNative?, options?, wrongNote?,
      // distractorNotes? }. When present, swap in the localized prompt/options/
      ...
      flat.push({
        ...
        grammar,
        wrongNote: (loc && loc.wrongNote) || (notes && notes.wrongNote) || null,
        distractorNotes: (loc && loc.distractorNotes) || (notes && notes.distractorNotes) || null,
