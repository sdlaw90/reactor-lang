...
export function flattenBank(bank) {
  ...
  Object.keys(bank).forEach((cat) => {
    bank[cat].forEach((q, i) => {
      flat.push({ id: `${cat}-${i}`, cat, prompt: q[0], options: q[1], correctIdx: q[2], explain: q[3] });
    });
  });
  return flat;
}
...
export function buildRound(track, mode, missedIds, seenAt) {
  ...
  Object.keys(track.bank).forEach((cat) => {
    ...
    const picks = pickFreshest(items, seenAt, perCat);
    picks.forEach((item) => blocks.push([shuffleOptions(item)]));
  });

  if (track.extraBank && track.extraBank.length > 0) {
    const pseudoItems = track.extraBank.map((_, i) => ({ id: `${track.extraCatId}-pair-${i}`, index: i }));
    const pairsPerRound = Math.min(track.extraPairsPerRound || 2, track.extraBank.length);
    const picks = pickFreshest(pseudoItems, seenAt, pairsPerRound);
    picks.forEach(({ index }) => {
