...
export function flattenBank(bank) {
  ...
  Object.keys(bank).forEach((cat) => {
    bank[cat].forEach((q, i) => {
      ...
      flat.push({ id: `${cat}-${i}`, cat, prompt: q[0], options: q[1], correctIdx: q[2], explain: q[3], difficulty: q[4] || null, promptEn: q[5] || null, promptNative: q[6] || null });
    });
  });
  return flat;
}
...
export function buildLessonSequence(track, catId) {
  ...
  const items = isExtra
    ...
    : flattenBank(track.bank).filter((item) => item.cat === catId);
