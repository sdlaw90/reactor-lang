...
// v3.4 (#60) — merge a side-table localized note into a base language map.
//
// Explanations / wrong-notes / distractor-notes are `{ en, es, pt, fr, … }` maps
// (a value may itself be a regional object, e.g. `{ latam, spain }`). Before
// v3.4 the ONLY way to add a source language was to edit the base bank in place,
// which is why `explain` is complete for en/es (baked in by v3.0/v3.1) and falls
// back to English for pt/fr. Side tables can now carry them too.
//
// `loc` is either a bare string — the SOURCE-language text, matching how the
// side tables already carry `prompt`/`promptNative` — or a partial map, for the
// rare item that needs more than one language or a regional split.
//
// The base map is SPREAD FIRST and never dropped, so `resolveExplainText`'s
// English fallback still fires for every item a side table hasn't reached yet.
// That is the difference from the pre-v3.4 `loc.wrongNote || base` shape, which
// replaced the whole map (and, given a bare string, would have handed the play
// screen a string where it expects a map). No side table used `wrongNote` before
// v3.4, so nothing ever shipped through that path.
export function mergeLangMap(base, loc, sourceLang) {
  if (loc == null) return base || null;
  const add = typeof loc === "string" ? { [sourceLang]: loc } : loc;
  return { ...(base || {}), ...add };
}

// Same merge, applied per distractor key. Distractor notes are keyed by OPTION
// TEXT, and a side table that localizes `options` re-keys them to the localized
// text — so base keys and localized keys do not correspond and the base map
// cannot be merged wholesale. When a side table supplies `distractorNotes` it
// therefore still replaces; each VALUE merges over the base value for the same
// key when one exists (tracks whose options are shared with the base).
function mergeDistractorNotes(base, loc, sourceLang) {
  if (loc == null) return base || null;
  const out = {};
  for (const k of Object.keys(loc)) out[k] = mergeLangMap(base && base[k], loc[k], sourceLang);
  return out;
}
...
export function flattenBank(bank, tagFor, sourceLang, l10nMap) {
  const flat = [];
  Object.keys(bank).forEach((cat) => {
    bank[cat].forEach((q, i) => {
      ...
      // Tagging always uses the canonical (base) prompt q[0], not a localized
      // one, so themes/grammar are language-agnostic.
      const tag = tagFor ? tagFor(q[0]) : null;
      // #89: fold the item-level `person` (yo/tú/…) onto the shared grammar
      // object so consumers read one field (q.grammar.person). Grammar objects
      // are shared across items, so we compose a fresh per-item copy rather than
      // mutate the shared tense definition. person is null when the blank
      // carries no person (infinitives, bare gerunds, pre-supplied auxiliaries).
      const grammar = tag && tag.grammar ? { ...tag.grammar, person: tag.person || null } : null;
      // #69: optional slot 7 — a hybrid wrong-answer note object
      //   { wrongNote?: { en, es }, distractorNotes?: { "<option text>": { en, es } } }.
      // wrongNote is a general "why a wrong pick is wrong / what to watch for"
      // shown on ANY incorrect answer; distractorNotes is optional per-distractor
      // and is keyed by OPTION TEXT (not index) so it survives shuffleOptions().
      // Both are null when absent, so existing 7-slot items are unaffected.
      const notes = q[7] && typeof q[7] === "object" ? q[7] : null;
      // v3.1 — per-source LOCALIZED SURFACE (side table), for "one track, many
      // sources." `l10nMap` (loaded per trackId+sourceLang from data/tracks/l10n)
      // maps item id ("cat-i") → { prompt?, promptNative?, options?, explain?,
      // wrongNote?, distractorNotes? }. When present, swap in the localized prompt/options/
      // notes so a Spanish speaker sees Spanish answer options / trad prompts /
      // notes instead of the English-facing originals. `options` MUST stay aligned
      // to the SAME indices as q[1] so correctIdx (q[2]) is unchanged;
      // distractorNotes are keyed by the LOCALIZED option text. Any field omitted
      // falls back to base content (graceful #60-style fallback), and English
      // natives (no l10nMap) are completely unaffected. Kept in SIDE FILES so the
      // bulk machine-generated localization never edits the giant banks in place.
      const loc = l10nMap ? l10nMap[`${cat}-${i}`] : null;
      const promptNative = loc && loc.promptNative
        ? { ...(q[6] || {}), [sourceLang]: loc.promptNative }
        : q[6] || null;
      flat.push({
        id: `${cat}-${i}`, cat,
        prompt: (loc && loc.prompt) || q[0],
        options: (loc && loc.options) || q[1],
        correctIdx: q[2],
        // v3.4 (#60): `explain` / `wrongNote` MERGE (base map first) rather than
        // replace, so an item the side table has not reached still resolves to
        // English instead of going blank.
        explain: mergeLangMap(q[3], loc && loc.explain, sourceLang),
        difficulty: q[4] || null,
        promptEn: q[5] || null,
        promptNative,
        themes: (tag && tag.themes) || [],
        grammar,
        wrongNote: mergeLangMap(notes && notes.wrongNote, loc && loc.wrongNote, sourceLang),
        distractorNotes: mergeDistractorNotes(
          notes && notes.distractorNotes,
          loc && loc.distractorNotes,
          sourceLang,
        ),
        // U4 dual-version card: { latam, spain } readings for a term that diverges
        // by region (celular/móvil…). Null unless the source surface tags this item.
        // Consumed on the explanation screen to teach the other region's form.
        variant: (loc && loc.variant) || null,
      });
    });
  });
  return flat;
  ...
  return [
    {
      ...
      wrongNote: item.identify.wrongNote || null,
      distractorNotes: item.identify.distractorNotes || null,
    },
    {
      id: `${catId}-${i}-b`,
      cat: catId,
      ...
      wrongNote: item.respond.wrongNote || null,
      distractorNotes: item.respond.distractorNotes || null,
    },
  ];
}
...
// #88 combined focus: minimum (category ∩ theme) matches before we build a
// blended round instead of falling back to the broader theme-only round. Kept
// export-shared so the picker UI gates on the exact same threshold the builder
// uses (no drift between what the chip promises and what the round delivers).
export const COMBINED_MIN = 4;

export function buildRound(track, mode, missedIds, seenAt, cefrSet, options = {}, sourceLang, l10nMap) {
  const flatBank = flattenBank(track.bank, track.tagFor, sourceLang, l10nMap);
  const flatExtra = track.extraBank ? flattenPairs(track.extraBank, track.extraCatId) : [];
  const flatAll = flatBank.concat(flatExtra);
  ...
  // #88 theme filter: pull every item tagged with the selected theme. On its
  // own a theme cuts ACROSS categories (every tagged item, from any category).
  // #88: when a category focus is ALSO active we intersect the two — e.g.
  // "Grammar items tagged Shopping" — instead of the old behaviour where the
  // theme silently overrode the category. Both paths still record to each
  // item's HOME category, so there's no separate theme mastery; level/freshness
  // bias still applies. Falls back to a normal round if nothing is tagged.
  ...
  // #88 combined focus: category ∩ theme. Requires COMBINED_MIN matches so we
  // never serve a one-question "round"; below that we fall through to the
  // theme-only round below (the theme is the narrower intent just expressed).
  if (themeFilter && hasCategoryFilter) {
    const catSet = new Set(options.categoryFilter);
    const pool = flatAll.filter(
      (item) => catSet.has(item.cat) && Array.isArray(item.themes) && item.themes.includes(themeFilter)
    );
    if (pool.length >= COMBINED_MIN) {
...
export function buildPlacementQuiz(track, perTier = 3, sourceLang, l10nMap) {
  const flatBank = flattenBank(track.bank, track.tagFor, sourceLang, l10nMap);
  const flatExtra = track.extraBank ? flattenPairs(track.extraBank, track.extraCatId) : [];
  const flatAll = flatBank.concat(flatExtra);
...
export function buildLessonSequence(track, catId, sourceLang, l10nMap) {
  const isExtra = catId === track.extraCatId;
  const items = isExtra
    ? flattenPairs(track.extraBank, track.extraCatId)
    : flattenBank(track.bank, track.tagFor, sourceLang, l10nMap).filter((item) => item.cat === catId);
...
// #88 combined-focus support for the picker UI: count tagged items at each
// (category, theme) intersection, using the SAME flatten + tagFor the round
// builder sees, so the start screen can show live viability and gate dead
// combos on real numbers. Returns { [catId]: { [themeId]: count } }; only
// tagged items contribute (untagged carry no themes).
export function themeCoverage(track) {
  const flat = flattenBank(track.bank, track.tagFor);
  const byCatTheme = {};
  flat.forEach((item) => {
    if (!Array.isArray(item.themes) || item.themes.length === 0) return;
    if (!byCatTheme[item.cat]) byCatTheme[item.cat] = {};
    item.themes.forEach((th) => {
      byCatTheme[item.cat][th] = (byCatTheme[item.cat][th] || 0) + 1;
    });
  });
  return byCatTheme;
}

// How many items a combined (category focus × theme) round would draw from,
// or null when a combined round doesn't apply (no theme, or no category focus
// so the theme just cuts across everything and isn't an intersection).
export function combinedPoolSize(coverage, categoryFilter, themeFilter) {
  if (!themeFilter || !Array.isArray(categoryFilter) || categoryFilter.length === 0) return null;
  return categoryFilter.reduce(
    (sum, cat) => sum + ((coverage[cat] && coverage[cat][themeFilter]) || 0),
    0
  );
}
