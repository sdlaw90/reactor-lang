    ...
    <div>
      ...
      <EmailSection session={session} setSession={setSession} />
        ...
        <div style={styles.comingSoonCard}>
          <p style={{ color: "#9B93B8", fontSize: 13, margin: 0 }}>Coming soon.</p>
        </div>
...
function NativeLanguageSection({ session, setSession }) {
  ...
  return (
    <Section title="Native / base language" saved={saved}>
      ...
      <p style={{ color: "#9B93B8", fontSize: 12, marginTop: 8 }}>
  ...
  if (!nativeLang) {
    return (
      <Section title="Native country">
        <p style={{ color: "#9B93B8", fontSize: 13 }}>Set your native language above first.</p>
      </Section>
    );
  }
    ...
    <Section title="Gameplay" saved={saved}>
      {!editing ? (
        <>
          ...
          <p style={{ color: "#9B93B8", fontSize: 12, marginTop: 8 }}>
            {currentPerCat} questions/category · {currentExtraPairs} phonetics pairs · question audio {currentQuestionAudio ? "on" : "off"} · answer-choice audio {currentChoiceAudio ? "on" : "off"} · tense hints {currentTenseHints ? "on" : "off"} · {currentQuestionTime}s
        ...
        <>
          ...
          <label style={{ ...styles.toggleRow, marginTop: 14 }}>
            ...
            <span>Show a speaker button on questions that have audio (tap to hear the prompt read aloud — never plays automatically)</span>
          </label>

          <label style={{ ...styles.toggleRow, marginTop: 14 }}>
            <input type="checkbox" checked={choiceAudio} onChange={(e) => setChoiceAudio(e.target.checked)} />
            <span>Show a speaker button on the answer choices too — only after you've answered (in review/pause), tap to hear an option read aloud. Rolling out per track; appears where option audio exists.</span>
          </label>

          <label style={{ ...styles.toggleRow, marginTop: 14 }}>
            <input type="checkbox" checked={tenseHints} onChange={(e) => setTenseHints(e.target.checked)} />
            <span>Tense training-wheels: on verb-conjugation questions, name the tense being asked for and why. On by default; you can also dismiss it in-round once you reach an advanced level.</span>
          </label>
