...
//
// Props:
//   steps       – array of { emoji, title, body } (already resolved to a language)
...
//   lang        – UI language for this component's own chrome (Step/Back/Next/
//                 Skip); "en" fallback. es strings pending native review (#41).

import { useEffect, useState } from "react";
import GuideDemo from "./GuideDemo";

// Chrome strings owned by this component (the step content comes in via `steps`).
const TOUR_UI = {
  en: {
    stepOf: (i, n) => `Step ${i} of ${n}`,
    goStep: (n) => `Go to step ${n}`,
...
export default function GuideTour({ steps, onDone, onSkip, doneLabel = "Let's go!", lang = "en" }) {
    ...
    <div style={styles.card}>
      <div style={styles.progressRow}>
        <span style={styles.counter}>
          {ui.stepOf(i + 1, steps.length)}
        </span>
        <div style={styles.dots}>
          {steps.map((s, idx) => (
            <button
              key={idx}
              aria-label={ui.goStep(idx + 1)}
              onClick={() => setI(idx)}
              style={{
                ...styles.dot,
                background: idx === i ? "#FFA6BE" : idx < i ? "#7E5C86" : "#3A3452",
