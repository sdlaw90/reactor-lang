...
function BigBenIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 90" width={size} height={size} style={{ flexShrink: 0 }}>
      <rect x="20" y="20" width="20" height="55" fill="#D3B0BF" />
      <rect x="16" y="10" width="28" height="12" fill="#D3B0BF" />
...
function GermanyFlagIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 40" width={size} height={size * (40 / 60)} style={{ flexShrink: 0 }}>
...
function RussiaFlagIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 40" width={size} height={size * (40 / 60)} style={{ flexShrink: 0 }}>
...
function JapanFlagIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 40" width={size} height={size * (40 / 60)} style={{ flexShrink: 0 }}>
...
function ChinaFlagIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 40" width={size} height={size * (40 / 60)} style={{ flexShrink: 0 }}>
      <rect x="0" y="0" width="60" height="40" fill="#DE2910" />
      <path d="M30,10 L32,16 L38,16 L33,20 L35,26 L30,22 L25,26 L27,20 L22,16 L28,16 Z" fill="#FFDE00" />
    </svg>
  );
}

function KoreaFlagIcon({ size = 32 }) {
  return (
    <svg viewBox="0 0 60 40" width={size} height={size * (40 / 60)} style={{ flexShrink: 0 }}>
      <rect x="0" y="0" width="60" height="40" fill="#FFFFFF" />
...
const ICONS_BY_TRACK_ID = {
  ...
  "de-for-en": GermanyFlagIcon,
  "ru-for-en": RussiaFlagIcon,
  "ja-for-en": JapanFlagIcon,
  "zh-for-en": ChinaFlagIcon,
  "ko-for-en": KoreaFlagIcon,
};

// Same id-keying trap as VARIANT_NAMES in languageNames.js: the icon depends on the
// TARGET, but the map is keyed by full track id, so `en-us-for-pt` matched nothing and
// the Portuguese English tiles rendered with no art at all. Derive a target-keyed index
// once so every future source language inherits the icons.
const SOURCE_SUFFIX = /-for-[a-z]{2}$/;
const ICONS_BY_TARGET = {};
Object.keys(ICONS_BY_TRACK_ID).forEach((id) => {
  const target = id.replace(SOURCE_SUFFIX, "");
  if (!ICONS_BY_TARGET[target]) ICONS_BY_TARGET[target] = ICONS_BY_TRACK_ID[id];
});

export default function TrackIcon({ trackId, size = 32 }) {
  const IconComponent =
    ICONS_BY_TRACK_ID[trackId] || ICONS_BY_TARGET[String(trackId || "").replace(SOURCE_SUFFIX, "")];
