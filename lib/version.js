...
export const CURRENT_VERSION = "2.23.0-beta.1";
...
export const CHANGELOG = [
  {
    ...
    date: "2026-07-08",
    changes: [
      "Adopted a version-suffix convention (-beta.N) for distinguishing beta from production releases, alongside a recommended dedicated beta git branch — a small BETA badge now shows next to the version number whenever a build carries the suffix",
      "Added a real browser-based E2E test suite (Playwright) covering the sign-in page, the beta application's multi-step navigation, and the exact selection-highlighting behavior a beta tester flagged — closing a real gap where verification had only ever been \"the build succeeds and pages return 200,\" which can't catch client-side JS crashes or broken interactive elements",
    ],
  },
  {
    version: "2.22.5",
    ...
    internal: true,
    changes: [
      "Found and fixed the real cause of the break-glass password tool's 500 error: createClient() calls in both admin API routes were outside their try/catch blocks, so any exception there (bad URL, bad key, network issue) crashed uncaught instead of returning a clean JSON error — defeating the earlier error-surfacing fix entirely. Wrapped everything that can throw in both routes, and hardened the admin pages' fetch handling the same way as the earlier fixes. Confirmed with a real reproduction: a broken config now returns a clean JSON error instead of an uncaught crash.",
    ],
  },
  {
    version: "2.22.4",
