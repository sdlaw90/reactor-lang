...
// VARIANT_NAMES is keyed by full track id, which means every new SOURCE language
// silently loses the regional qualifier: `en-us-for-pt` is not `en-us-for-es`, so a
// Portuguese speaker saw two tracks both called "Inglês" with no US/UK distinction.
// The displayed name depends only on the TARGET, so derive a target-keyed index once
// (strip the `-for-<src>` suffix) and fall back to it. New source languages inherit
// every variant name automatically — no per-source entries to remember.
const SOURCE_SUFFIX = /-for-[a-z]{2}$/;
const VARIANT_NAMES_BY_TARGET = {};
Object.keys(VARIANT_NAMES).forEach((id) => {
  const target = id.replace(SOURCE_SUFFIX, "");
  if (!VARIANT_NAMES_BY_TARGET[target]) VARIANT_NAMES_BY_TARGET[target] = VARIANT_NAMES[id];
});
...
export function trackDisplayName(track, viewerNativeLang) {
  ...
  const byTarget = VARIANT_NAMES_BY_TARGET[String(track.id).replace(SOURCE_SUFFIX, "")];
  if (byTarget) {
    return byTarget[lang] || byTarget.en || track.label;
  }
