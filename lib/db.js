...
export async function submitFeedback(userId, type, message, pageContext) {
  const { error } = await supabase
    .from("feedback_submissions")
    .insert({ user_id: userId, type, message, page_context: pageContext || null });
  if (error) throw error;
}

export async function submitBetaApplication(name, email, reason, languagesInterested) {
  const { error } = await supabase
    .from("beta_applications")
    .insert({ name, email, reason: reason || null, languages_interested: languagesInterested || null });
  if (error) throw error;
}
