...
// RETIRED: the in-app beta survey this powered has been replaced by the
// bug-report / feature-request flows below (the survey now goes out by email
// at the end of each beta round). Kept only in case another page still
// imports it -- safe to delete once confirmed unused.
...
// RETIRED 2026-07-10: app/beta-apply/page.js now calls /api/beta-apply
// directly (auto-approve flow). No remaining callers — delete alongside
// submitFeedback once confirmed unused.
...
// ---------- bug reports & feature requests (server route) ----------
// Both go through /api/submit-feedback so the server can derive the
// sender's identity from their session token (never typed, never spoofable),
// store the screenshot in the private bucket, and notify the admin.

async function postFeedbackForm(formData) {
  const { data } = await supabase.auth.getSession();
  const token = data?.session?.access_token;
  if (!token) throw new Error("You need to be signed in to send this.");
...
export async function submitBugReport({ message, steps, errorCode, screenshot }) {
  const formData = new FormData();
  formData.set("type", "bug");
  formData.set("message", message);
  if (steps) formData.set("extra", steps);
  if (errorCode) formData.set("errorCode", errorCode);
  if (screenshot) formData.set("screenshot", screenshot);
  if (typeof document !== "undefined" && document.referrer) formData.set("pageContext", document.referrer);
  if (typeof navigator !== "undefined") formData.set("userAgent", navigator.userAgent);
  await postFeedbackForm(formData);
}

export async function submitFeatureRequest({ message, problem }) {
  const formData = new FormData();
  formData.set("type", "feature");
  formData.set("message", message);
  if (problem) formData.set("extra", problem);
  if (typeof navigator !== "undefined") formData.set("userAgent", navigator.userAgent);
  await postFeedbackForm(formData);
}
