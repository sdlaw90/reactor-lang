...
// A deploy shows a newer /version.json the instant Vercel finishes, but the
// rest of the release chain (DB migrations, TTS audio sync, smoke-check) runs
// separately in CI and takes longer. Prompting on version.json alone can nudge
// someone to reload into code whose audio isn't in prod yet. So we gate the
// prompt on a second signal: a CI-written marker that advances to the new
// version ONLY after the whole chain passes. Same public bucket + fetch the
// audio manifest already loads, so CORS is a solved problem here.
const RELEASE_READY_URL = process.env.NEXT_PUBLIC_SUPABASE_URL
  ? `${process.env.NEXT_PUBLIC_SUPABASE_URL.replace(/\/$/, "")}/storage/v1/object/public/tts-audio/release-ready.json`
  : null;

// Returns the version the release chain has fully published, or null if the
// marker is missing/unreadable. Fail-closed by design: null means "hold the
// prompt", because publish-ready is a required, smoke-gated CI step — no
// marker at the deployed version means the release isn't actually live yet.
async function fetchReleaseReadyVersion() {
  if (!RELEASE_READY_URL) return null;
  try {
    const res = await fetch(`${RELEASE_READY_URL}?t=${Date.now()}`, { cache: "no-store" });
  ...
  useEffect(() => {
    const checkVersion = async () => {
      try {
        ...
        if (!deployed || deployed === CURRENT_VERSION) return;

        // A newer version is deployed. Only surface the prompt once the release
        // chain has fully published that same version (see marker note above).
        // If the marker is behind (Vercel deployed ahead of CI) or unreadable,
        // hold and re-check on the next poll — this is the whole point.
        const readyVersion = await fetchReleaseReadyVersion();
        if (readyVersion === deployed) {
